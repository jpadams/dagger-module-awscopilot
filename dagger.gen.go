...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Awscopilot":
		...
		case "Base":
			...
			err = json.Unmarshal([]byte(inputArgs["copilotVersion"]), &copilotVersion)
			if err != nil {
				fmt.Println(err.Error())
				os.Exit(2)
			}
			return (*Awscopilot).Base(&parent, ctx, alpineVersion, copilotVersion)
		case "Test":
			var err error
			var parent Awscopilot
			err = json.Unmarshal(parentJSON, &parent)
			...
			var awsid string
			err = json.Unmarshal([]byte(inputArgs["awsid"]), &awsid)
			if err != nil {
				fmt.Println(err.Error())
				os.Exit(2)
			}
			var awssecret string
			err = json.Unmarshal([]byte(inputArgs["awssecret"]), &awssecret)
			if err != nil {
				fmt.Println(err.Error())
				os.Exit(2)
			}
			...
			return (*Awscopilot).Test(&parent, ctx, awsid, awssecret, awsregion)
		case "":
			var err error
			var typeDefBytes []byte = []byte("{\"asObject\":{\"functions\":[{\"args\":[{\"name\":\"alpineVersion\",\"typeDef\":{\"kind\":\"StringKind\"}},{\"name\":\"copilotVersion\",\"typeDef\":{\"kind\":\"StringKind\"}}],\"name\":\"Base\",\"returnType\":{\"asObject\":{\"name\":\"Container\"},\"kind\":\"ObjectKind\"}},{\"args\":[{\"name\":\"awsid\",\"typeDef\":{\"kind\":\"StringKind\"}},{\"name\":\"awssecret\",\"typeDef\":{\"kind\":\"StringKind\"}},{\"name\":\"awsregion\",\"typeDef\":{\"kind\":\"StringKind\"}}],\"name\":\"Test\",\"returnType\":{\"kind\":\"StringKind\"}}],\"name\":\"Awscopilot\"},\"kind\":\"ObjectKind\"}")
